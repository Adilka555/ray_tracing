...
vec4 CTracer::lookupDt (float x, float y)
{ //измененный корел. диск помещаем на изображение
    int W, H;

    int i, j;
    W = ImgDisk.width, j;
    H = ImgDisk.height, i;
    int chn = ImgDisk.channels;
  ...
  auto pd = ImgDisk.data;
  //чтобы определить нужное положение в дэйта.
  pd += chn * (W*i + j);

  auto r = pd[0], g = pd[1], b = pd[2], al = pd[3]; //ал - прозрачность.
  float r_f, g_f, b_f, al_f;
...
vec3 CTracer::lookupSt (float x,float y)
{   // Размещаем вектор на изображении
...
void CTracer::RenderImage () {
  ...
  for(int i = 0; i < yRes; ++i){
    for(int j = 0; j < xRes; ++j){
      ...
      m_camera.m_pixels[i * xRes + j] = TraceRay(ray); //узнали куда попал фотон
    }
  }
}
void CTracer::SaveImageToFile(std::string fileName){
  ...
  for(int i = 0; i < h; ++i){
    for(int j = 0; j < w; ++j){
      vec3 color = m_camera.m_pixels[pxlDispl + j];
